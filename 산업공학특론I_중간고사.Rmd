...
draw_data <- function(x, title){ #x : 변수
  if(is.factor(x)) { #범주형 변수 확인
    barplot(table(x), col = 'blue', main = title, las = 1) #막대그래프, table():빈도
  }
  else {
    hist(x, breaks = 20, col = 'blue', main = title, xlab = 'Value') #히스토그래프
  }
}
...
fit_dist <- function(x){
  ...
  fit_result <- matrix(nrow = 3, ncol = length(fitlist))
  colnames(fit_result) <- fitlist
  rownames(fit_result) <- c('P1', 'P2', 'loglik')

  for(i in 1:length(fitlist)){
    fit_temp <- fitdist(dat$actual_productivity, fitlist[i]) # MLE 추정 함수 사용
    ...
    fit_result[,i] = c(est_temp, ll_temp)
  }
  print(fit_result) # 모수 추정 결과 도출
}
