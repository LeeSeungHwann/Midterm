...
#dat <- dat[,-(6:7)]
#dat <- dat[,-(8:11)]
dat$department <- gsub(' ','',dat$department)
...
```
#### -> 추정 결과에 따라 최적의 분포로 '정규 분포' 도출.
###### : 정규분포에서 가장 큰 로그우도 값을 보이고, AIC, BIC 값이 가장 작은 값을 보이고 있으므로 최적의 분포로 도출됨.
...
hist(x, freq = FALSE, col = 'grey', breaks = 5, prob = T, main = 'Density of Productivity Data', xlab = "Actual Productivity", ylab = "Density")
curve(dnorm(x, mean = fit_mean, sd = fit_sd, log=F), col = 'blue', add = T, lwd = 2, yaxt = "n")
